...
from sqlalchemy import select, func as sql_func, distinct

from core.database.models import News, Event, Source
...
class ImportanceScoreCalculator:
    ...
    async def _calculate_novelty_score(self, event: Event) -> float:
        ...
        if company_tickers:
            ...
            for ticker in company_tickers[:3]:  # Ограничиваем до 3 тикеров
                # Подсчитываем события того же типа с тем же тикером
                # В SQLite attrs хранится как TEXT (JSON), поэтому используем LIKE
                stmt = select(sql_func.count()).select_from(Event).where(
                    Event.event_type == event_type,
                    Event.ts >= lookback_date,
                    Event.ts < event.ts,
                    Event.attrs.like(f'%{ticker}%')  # Простой поиск для SQLite
                )
    ...
    async def _calculate_burst_score(self, event: Event) -> float:
        ...
        stmt = select(sql_func.count()).select_from(Event).where(
            Event.event_type == event.event_type,
            Event.ts >= recent_start,
            Event.ts <= event.ts
        )

        recent_count_result = await self.session.execute(stmt)
    ...
    async def _calculate_credibility_score(self, event: Event) -> float:
        ...
        if event.news_id:
            from core.database.models import News, Source

            stmt = select(Source.trust_level, Source.kind).select_from(News).join(
                Source, News.source_id == Source.id
            ).where(News.id == event.news_id)

            source_result = await self.session.execute(stmt)
    ...
    async def _check_source_correlation(self, event: Event) -> float:
        ...
        try:
            ...
            stmt = select(
                sql_func.count(distinct(Source.kind)),
                sql_func.count()
            ).select_from(Event).join(
                News, Event.news_id == News.id
            ).join(
                Source, News.source_id == Source.id
            ).where(
                Event.event_type == event.event_type,
            ...
            corr_result = await self.session.execute(stmt)
            unique_sources, total_events = corr_result.first() or (0, 0)
